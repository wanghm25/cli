...
func (g *fakeCreateGateway) WalkSubscriptions(_ context.Context, _ subown.ListParams, visit func(model.RemoteSubscription) bool) (bool, error) {
	call := g.walkCalls
	g.walkCalls++
...
func (g *fakeCreateGateway) Create(_ context.Context, spec subown.CreateSpec) (*model.RemoteSubscription, error) {
	g.createCalls++
	s := spec
	g.createSpec = &s
	if g.createErr != nil {
...
// Get and Renew satisfy the widened subscription.Gateway surface. The create
// command's Plan/Apply flow never invokes them, so these are inert stubs.
func (g *fakeCreateGateway) Get(_ context.Context, _ string) (*model.RemoteSubscription, error) {
	return nil, nil
}

func (g *fakeCreateGateway) Renew(_ context.Context, _ string) (*model.RemoteSubscription, error) {
	return nil, nil
}
