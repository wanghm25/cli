...
deleting the remote Subscription is NOT a substitute for stopping a 
local 'event consume' process.

IDENTITY:
--as user|bot|auto, resolved to one effective identity (no
...
OUTPUT:
{operation, remote_subscription_id, deleted: true, subscription{...last known state...}, next_action}.

SAFETY:
this is a high-risk write on a resource other identities/processes
