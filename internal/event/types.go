...
// RawEvent is the cross-cutting event value object, now owned by
// internal/event/model. It is aliased here so existing callers keep using
// event.RawEvent unchanged.
type RawEvent = model.RawEvent

// APIClient is the runtime OAPI interface, now owned by
// internal/event/model. It is aliased here so existing callers keep using
// event.APIClient unchanged.
type APIClient = model.APIClient
