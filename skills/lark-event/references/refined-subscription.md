...
**Rotation / enable / disable = delete + recreate.** `encrypt` is Create-only: it can never be added, changed, or removed afterward — and the platform's update (Patch) API is filter-only now, carrying no `payload_options` field at all, so it is structurally incapable of touching `include_resource_data` in either direction. `event subscription update --include-resource-data=true` **and** `--include-resource-data=false` are therefore both refused, identically, with a typed `failed_precondition` guiding you to delete + recreate (or create a separate new subscription) after human confirmation — purely locally, with no remote read. There is no in-place rotate/update-key.
