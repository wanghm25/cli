...
**Rotation / enable / disable = delete + recreate.** `encrypt` is Create-only: it can never be added, changed, or removed afterward — and the platform's update (Patch) API is filter-only now, carrying no `payload_options` field at all, so it is structurally incapable of touching `include_resource_data` in either direction. `event subscription update` therefore carries no `--include-resource-data` flag at all — it changes only the server-side filter — so changing resource-data delivery is only ever done by delete + recreate (or a separate new subscription) after human confirmation. There is no in-place rotate/update-key.
