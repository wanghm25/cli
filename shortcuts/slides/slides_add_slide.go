...
var SlidesAddSlide = common.Shortcut{
	...
	Tips: []string{
		...
		"Adding pages in a shell loop: one page failing does not stop the loop, so check every result rather than the loop's exit code.",
	},
...
func addSlideQuery(runtime *common.RuntimeContext) map[string]interface{} {
	query := map[string]interface{}{"revision_id": runtime.Int("revision-id")}
